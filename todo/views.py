...
def signupuser(request):
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:  # удачная регистрация
                user = User.objects.create_user(request.POST['username'], password=request.POST['password1'])
                user.save()
                login(request, user)
                return redirect('currenttodos')
            except IntegrityError:
                return render(request, 'todo/signupuser.html', {'form': UserCreationForm(), 'error': 'This username has '
                                                                                                'already been taken. '
                                                                                                'Please choose a new '
                                                                                                'username'})
        else:
...
def loginuser(request):
    if request.method == 'GET':  # вход зареганного юзера
        return render(request, 'todo/loginuser.html', {'form': AuthenticationForm()})
    else:
        user = authenticate(request, username=request.POST['username'], password=request.POST['password'])
        if user is None:  # если не существует или неправильный пароль (соотв. данных нет)
...
def viewtodo(request, todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            form = TodoForm(request.POST, instance=todo) # изменяем уже сущ
            form.save()
            return redirect('currenttodos')
        except ValueError:
